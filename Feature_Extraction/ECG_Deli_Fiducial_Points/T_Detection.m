...
for isoP=1:1:sum(isnan(isoPosition(:,1)))
    ...
    dIso=mean(diff(isoPosition(max(correctionIsoPos(isoP)-6,1):min(correctionIsoPos(isoP)-1,length(isoPosition)),1)));
...
for i=1:size(FPT,1)
    ...
    if L_width<2 || R_width<2 || TPEAK_vector(i)+R_width>length(Dx)
        ...
        elseif loco_off_lvl(1)+round(0.04*samplerate)<length(signal) && loco_off_lvl1(1)+round(0.04*samplerate)<length(signal) && abs(loco_off_lvl(1)-TOFF(i))<round(0.02*samplerate) && abs(sum(diff(signal(loco_off_lvl(1):loco_off_lvl(1)+round(0.04*samplerate)))))<3*abs(sum(diff(signal(loco_off_lvl1(1):loco_off_lvl1(1)+round(0.04*samplerate))))) || abs(TPEAK_vector(i)-loco_off_lvl1(1))<0.6*mean(TOFF_vector-TPEAK_vector) || abs(TPEAK_vector(i)-loco_off_lvl1(1))>1.4*mean(TOFF_vector-TPEAK_vector)
            TOFF_vector(i)=loco_off_lvl(1);
